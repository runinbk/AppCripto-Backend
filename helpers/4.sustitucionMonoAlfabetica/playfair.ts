export function cifradoPlayfair(texto: string, clave: string): string {
...
function generarMatrizClave(clave: string): string[][] {
...
function obtenerPares(texto: string): string[] {
    const pares: string[] = [];
...
function cifrarPar(matriz: string[][], par: string): string {
    const [a, b] = par.split('');
...
function encontrarPosicion(matriz: string[][], letra: string): [number, number] {
    for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 5; j++) {
            if (matriz[i][j] === letra) {
                return [i, j];
...
export function descifradoPlayfair(textoCifrado: string, matriz: string[][]): string {
...
function descifrarPar(matriz: string[][], par: string): string {
    const [a, b] = par.split('');
    const [filaA, colA] = encontrarPosicion(matriz, a);
    const [filaB, colB] = encontrarPosicion(matriz, b);
